...
EXAMPLE_CONFIG = {
    "智慧座艙 AR-HUD": {
     ...
     "智能客服技術": {
        "main_boolean": "(((人工智慧 OR AI OR \"ARTIFICIAL INTELLIGENCE\" OR 自然語言 OR NL OR \"NATURAL LANGUAGE\" OR ((機器學習 OR ML OR \"MACHINE LEARNING\" OR 深度學習 OR DL OR \"DEEP LEARNING\" OR SVM OR \"Support Vector Machine\" OR \"Random Forest\" OR Adaboost OR \"Decision Tree\" OR \"Markov Decision Process\" OR KNN OR \"k-means\" OR \"Naive Bayes\" OR \"tf-idf\" OR 神經網路 OR \"Neural Network\" OR RNN OR \"Recurrent Neural Network\" OR LSTM OR \"Long Short-Term Memory\" OR GRU OR \"Gated Recurrent Unit\" OR \"Generative Adversarial Networks\" OR GAN OR Autoencoder OR BERT OR \"Bidirectional Encoder Representations from Transformers\" OR ROBERTA OR \"A Robustly Optimized BERT Pretraining Approach\" OR \"Self-Attention\" OR \"Multi-Head Attention\" OR XLNet OR ELECTRA OR ELMO OR T5 OR GPT OR \"Generative Pre-trained Transformer\" OR LLaMA OR \"Large Language Model Meta AI\" OR LLM* OR ChatGPT)) AND ((識別 OR IDENTIF* OR 分析 OR ANALY* OR 分類 OR Classif* OR CATEGOR* OR 標記 OR Label* OR 命名 OR Nameing OR NER OR 生成 OR Generat* OR 翻譯 OR Translat* OR 轉錄 OR Transcrip* OR 問答 OR Question* OR 回覆 OR 回應 OR 回答 OR REPLY OR RESPONSE OR ANSWER*) AND ((文字 OR 文本 OR 情感 OR 情緒 OR text* OR emotion* OR senti*) OR (語音 OR 聲音 OR 音頻 OR VOICE OR AUDIO) OR (圖像 OR 圖片 OR 影像 OR 照片 OR IMAGE* OR PHOTO* OR PICTURE*))))) AND (客服 OR 客戶服務 OR \"CUSTOMER SERVICE\" OR 客戶 OR 顧客 OR 用戶 OR 使用者 OR CUSTOMER OR USER) AND (對話 OR DIALOGUE OR 聊天 OR CHAT OR 助理 OR 助手 OR 客訴 OR 秘書 OR ASSIST* OR CONVERSATION*)) @TI,AB,CL",
        "matrix": {
            "technologies": [
                {
    ...
    "台灣大哥大智能客服-語音辨識技術": {
        "main_boolean": "((深*神經網* OR 深度學習 OR 機器學習 OR (deep NEAR/10 neural network*)) AND (自動語音 OR speech OR ASR OR TTS OR \"text to speech\")) AND ((語音 NEAR/3 (檢測 OR 偵測) OR 聲音 NEAR/3 (檢測 OR 偵測)) OR ((語音 OR 音訊 OR 音聲) AND (生成 OR 合成)) OR ((情感 OR 情緒 OR emotion*) AND (計算 OR 分析 OR 識別 OR computing)))",
    "matrix": {
        "technologies": [
            {
            "label": "深度神經網絡 (DNN)",
            "boolean": "(深[1,4]神經網路 OR 深度學習 OR 強化學習 OR 機器學習 OR deep neural network OR deep learning OR machine learning)@TI,AB,CL"
            },
            {
            "label": "語音活動檢測 (VAD)",
            "boolean": "(語音[1,3]檢測 OR 聲音[1,3]檢測 OR VAD OR Voice Activity Detection OR speech activity detection)@TI,AB,CL"
            },
            {
            "label": "語音合成 (Speech Synthesis)",
            "boolean": "((語音 OR 音訊 OR 音声 OR 音頻) AND (產生 OR 合成 OR 生成)) OR (speech AND synthe*) OR スピーチシンセサイザー@TI,AB,CL"
            },
            {
            "label": "情感計算 (Affective Computing)",
            "boolean": "((情感 OR 情緒 OR 感情) AND (計算 OR 識別 OR 分析 OR 認識)) OR ((affective OR emotion) AND (computing OR recognition))@TI,AB,CL"
            },
            {
            "label": "語音分離 (Speech Separation)",
            "boolean": "((語音 OR 聲音 OR 音声) AND (分離 OR 隔離 OR 分割)) OR ((voice OR speech) AND separat*)@TI,AB,CL"
            },
            {
            "label": "語者分割 (Speaker Diarization)",
            "boolean": "((語者 OR 說話人) AND (分割 OR 分離 OR 切割 OR 識別)) OR (speaker AND (Diarization OR segmentation OR separation))@TI,AB,CL"
            }
    ]   ,
    "efficacies": [
        {
          "label": "迴聲消除 (Echo Cancellation)",
          "boolean": "((回聲 OR 回波 OR 回音) AND (消除 OR 去除)) OR Echo Cancel*@TI,AB,CL"
        },
        {
          "label": "噪聲抑制 (Noise Reduction)",
          "boolean": "((噪聲 OR 噪音 OR 雜訊) AND (抑制 OR 限制 OR 降低)) OR 去噪 OR 降噪 OR (Noise AND (Reduc* OR Suppress*))@TI,AB,CL"
        },
        {
          "label": "公平性 (Fairness)",
          "boolean": "(公平性 OR 公平 OR Fairness)@TI,AB,CL"
        },
        {
          "label": "實時處理 (Real-time Processing)",
          "boolean": "((實時 OR 即時) AND 處理) OR Real-time Processing@TI,AB,CL"
        },
        {
          "label": "隱私保護 (Privacy Protection)",
          "boolean": "((隱私 OR プライバシー) AND 保護) OR Privacy Protect*@TI,AB,CL"
        },
        {
          "label": "能效優化 (Energy Efficiency)",
          "boolean": "((能效 OR 能源效率) AND 優化) OR Energy Efficiency@TI,AB,CL"
        }
      ]
    }
  },
    ...
    "口服巴金森氏症藥物 (Oral Parkinson's Disease Drugs)": {
        ...
        "matrix": {
            ...
            "efficacies": [
                ...
                {
                "label": "疾病修飾/延緩進程 (Disease Modifying)",
                "boolean": "(\"Disease Modifying\" OR neuroprotection OR \"slow progression\" OR \"delay progression\" OR \"stop progression\" OR 神經保護 OR 延緩病程)"
                }
            ]
        }
    },
